...
class Net {
    constructor(props) {
        ...
        const neuronConfig = Helper.validate(this.props.neuronConfig);
        ...
        if (this.props.hiddenLayers == 0)
            this.net.push(new Layer({numOfInputs: this.props.numOfInputs, numOfNeurons: this.props.numOfOutputs, neuronConfig}));
        else {
            this.net.push(new Layer({numOfInputs: this.props.numOfInputs, numOfNeurons: this.props.hiddenLayers, neuronConfig}));
            
            let layers = this.props.hiddenLayers;
            while(layers--)
                this.net.push(new Layer({numOfInputs: this.props.hiddenLayers, numOfNeurons: this.props.hiddenLayers, neuronConfig}));
            
            this.net.push(new Layer({numOfInputs: this.props.hiddenLayers, numOfNeurons: this.props.numOfOutputs, neuronConfig}));
        }
    }
