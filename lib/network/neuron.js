...
class Neuron {
    constructor(props) {
        ...
        this.weights = _.map(new Array(this.props.numOfInputs), () => Math.random() * 2 - 1);
    ...
    ignite(inputs) {
        ...
        this.lastActivation = Neuron.sigmoid(sum);
    ...
    deserializeGenome(gene) {
        ...
        if (gene.length == this.props.numOfInputs) {
            console.log('Warning, deserializing a genome without a bias. Is this intentional?'.yellow);
    ...
    toString() {
        var str = this.weights.map(weight => weight.toFixed(2)).join(', ') + ' || ' + this.bias.toFixed(2);
