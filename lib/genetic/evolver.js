...
class Genetic {
    constructor(props) {
        ...
        Config.configure(this.props.Person.neuronConfiguration());
    ...
    procreate(parents) {
        ...
        for (let i = 0; i < genomeLength; i++) {
            if (this.props.mutationProbability > Math.random())
                sonGenome[i] += this.props.maximumMutation * Config.neuronInit();
            if (this.props.mutationProbability > Math.random())
                daughterGenome[i] += this.props.maximumMutation * Config.neuronInit();
        }
    ...
    testAndPrintPopulation(inputs) {
        let prettyOutput = 
            ...
            .map(obj => obj.person.name + ' => ' + obj.outputs.map(out => out.toFixed(Config.printPrecision)))
            .join('\n');
