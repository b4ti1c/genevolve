...
const debug = require('debug')('genetic:evolver');
...
class Genetic {
    ...
    evolveGeneration() {
        debug('Evolving a new generation');
    ...
    selectRandomParents() {
        ...
        } while (selectionProbability < Math.random() || mom == dad); //|| this.partners[mom.name][dad.name]);
    ...
    procreate(parents) {
        ...
        for (let i = 0; i < genomeLength; i++) {
            if (this.props.mutationProbability > Math.random())
                sonGenome[i] += this.props.maximumMutation * this.props.Person.generateGene() * Math.pow(-1, Math.floor(Math.random() * 2));
            if (this.props.mutationProbability > Math.random())
                daughterGenome[i] += this.props.maximumMutation * this.props.Person.generateGene() * Math.pow(-1, Math.floor(Math.random() * 2));
        }
    ...
    testAndPrintPopulation(inputs, opt_out) {
        let prettyOutput = 
            ...
            .map(obj => obj.person.name + ' => ' + 
                        obj.outputs.map(out => out.toFixed(this.props.Person.getPrintPrecision())) + 
                        (opt_out ? ' --- ' + obj.person.getFitness({inputs, outputs: opt_out}) : ''))
            .join('\n');
